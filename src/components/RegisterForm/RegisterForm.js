...
export const RegisterForm = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!validateUsername(name)) {
      validationError.name = "Имя пользователя должно содержать минимум 3 символа";
    }

    if (!validateEmail(email)) {
      validationError.email = "E-mail должен содержать минимум 3 символа перед @";
    }

    if (!validatePassword(password)) {
      validationError.password = "Пароль должен содержать минимум 7 символов";
    }
